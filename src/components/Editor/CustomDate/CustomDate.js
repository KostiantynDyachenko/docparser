...
import CalendarIconString from "../icons/CalendarIconString";
...
/**
 * Base CustomDate Block for the Editor.js.
 * Represents simple CustomDate
 */

/**
 * @typedef {object} CustomDateConfig
 * @property {string} placeholder - placeholder for the empty CustomDate
 * @property {boolean} preserveBlank - Whether or not to keep blank CustomDates when saving editor data
 */

/**
 * @typedef {Object} CustomDateData
 * @description Tool's input and output data format
 * @property {String} text — CustomDate's content. Can include HTML tags: <a><b><i>
 */
class CustomDate {
    /**
     * Default placeholder for CustomDate Tool
     *
     * @return {string}
     * @constructor
     */
     ...
     * @param {CustomDateData} params.data - previously saved data
     * @param {CustomDateConfig} params.config - user config for Tool
    ...
    constructor({data, config, api, readOnly, ...rest}) {
        ...
        /**
         * Placeholder for CustomDate if it is first Block
         * @type {string}
         */
        this._placeholder = config.placeholder ? config.placeholder : CustomDate.DEFAULT_PLACEHOLDER;
        this._data = {};
        this._element = this.drawView();
        [this._dateWrapper, this._dateElement1, this._dateElement2] = this.drawTime();
    ...
    renderSettings() {
        ...
        this.settings.forEach( tune => {
            ...
            button.addEventListener('click', (event) => {
                ...
                let [x, y] = this.getOffsetXY(this._element);
                let height = this._element.offsetHeight;
                let width = this._element.offsetWidth;
                this._toggleTune(tune.name, button);
                if (this?.config[tune.name]) {
                    this.config[tune.name](this._getTuneProps(tune.name), () => {this._toggleTune(tune.name, button)}, { x, y, height, width });
                }
            });

            button.addEventListener('mouseenter', () => {
                this.api.tooltip.show(button, tune.title);
            });
    ...
    getOffsetXY(element, x = 0, y = 0) {
        ...
        //     return this.getOffsetXY(element.offsetParent, x, y);
        // }
        // else {
        //     return [x, y];
        // }
    ...
    drawTime() {
        ...
        div.classList.add("editor-date-time");
        div.contentEditable = false;
    ...
    render() {
        ...
        wrapper.append(this._dateWrapper);
    ...
    /**
     * Validate CustomDate block data:
     * - check for emptiness
     *
     * @param {CustomDateData} savedData — data received after saving
     ...
     * @param {HTMLDivElement} toolsContent - CustomDate tools rendered view
     * @returns {CustomDateData} - saved data
     * @public
     */
    save(toolsContent) {
    ...
    /**
     * Enable Conversion Toolbar. CustomDate can be converted to/from other tools
     */
    static get conversionConfig() {
        return {
            export: 'text', // to convert CustomDate to other block, use 'text' property of saved data
            import: 'text' // to covert other block's exported string to CustomDate, fill 'text' property of tool data
        };
    }
     ...
     * @returns {CustomDateData} Current data
     * @private
     */
    get data() {
     ...
     * - at the HTML
     *
     * @param {CustomDateData} data — data to set
     * @private
     */
    set data(data) {
        this._element.innerHTML = "";
        ...
        this._dateElement1.innerHTML = data.date;
        //this._dateElement1.innerHTML = data.time_line_header;
    }
